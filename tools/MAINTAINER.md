...
- `drive.ts` — the headless UI driver: launches installed Edge through
  ...
  drags — including split `down`/`move`/`up` for mid-drag screenshots —
  wheel), writes screenshots to `out/ui/`, and fails on page console
  errors (ignoring the expected favicon 404).
